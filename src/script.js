...
//-------------------------------------------\/Добавление класса fix(Фиксирование навигации), когда пользователь проскролил header\/-------------------------------------------
window.onscroll = function navFix(){
    let nav = document.querySelector('nav'); //Запись тега nav в переменную nav
    if(window.pageYOffset>=document.querySelector("nav").offsetHeight){ //Проверка на то, что экран прокручен на 100px
